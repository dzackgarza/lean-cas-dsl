...
The primary objects are categories and the functors between them; each
`anchor` says which category (or defining constant) in Mathlib an entry
means, and each `parents` entry is an inclusion functor. What the registry
stores is the name-graph EXTRACTED from that diagram — a derivative
encoding the transitional resolver can walk; params ride along edges
unchanged. Nothing here names an implementation, a backend, or a current
capability limit. The docs are user-facing — they are what `#capabilities`
prints. -/

private def stdCategories : Array CatDecl := #[
  { name := `Sets, anchor := ``CategoryTheory.types,
...
enumeration is registered" },
  ...
  { name := `FactorizationElems, parents := #[`CommRingElems],
    ...
    anchor := ``UniqueFactorizationMonoid,
...
by one element" },
  { name := `EuclideanElems, parents := #[`PIDElems],
    ...
    anchor := ``EuclideanDomain,
    doc := "elements of a euclidean domain: a division algorithm, hence \
also a PID and a UFD" },
  { name := `Modules, anchor := ``ModuleCat,
    paramTelescope := #[``_root_.Module],
    doc := "modules over a commutative ring" },
  { name := `CyclicModules, parents := #[`Modules],
    anchor := ``Submodule.IsPrincipal,
    paramTelescope := #[``_root_.Module, ``CasDsl.Anchors.IsCyclicModule],
    doc := "cyclic modules over a commutative ring: generated by a single \
element, $M = Rx$ — the full subcategory of modules with $\\top$ principal" },
  { name := `MatrixElems, anchor := ``Matrix,
  ...
  { name := `PolynomialElems, anchor := ``Polynomial,
  ...
  { name := `FunctionElems, anchor := ``TypeCat.Fun,
  ...
  { name := `ComplexElems, anchor := ``Complex,
    doc := "elements of ℂ, or of a subfield presented here (ℝ): the real and \
imaginary parts, conjugation and the modulus; params (the domain the element \
was presented in)" },
  ...
  { name := `«Schemes/QQ», anchor := ``AlgebraicGeometry.Scheme,
    doc := "schemes over ℚ — the over-category of $\\operatorname{Spec} \\mathbb{Q}$ \
in the category of schemes. SPEC.md's own name for the category an affine \
scheme `Spec R` is ascribed to" },
  { name := `«QQ-Mod», anchor := ``ModuleCat,
    doc := "ℚ-modules — Mathlib's category of modules over ℚ. SPEC.md's own \
name for the category a subspace of ℚⁿ is a subobject of; owns `dim`" },
  ...
  { name := `HomElems, anchor := ``LinearMap,
...
once, on `Modules` — it is defined for any module — and reaches the cyclic
fixture through `CyclicModules ≤ Modules`, with no forwarding declaration.
That the shipped implementation answers only cyclic ℤ-modules is route data
(§4): the membership states mathematics, the route states computability. -/
...
$-7$ is prime exactly as $7$ is, $(-7) = (7)$" },
  ...
  { id := `derivative, receiver := `PolynomialElems,
    ...
    doc := "the formal derivative d/dx, by exact coefficient arithmetic — \
computed natively" },
...
a coset of ker(d/dx) — computed natively where the divisions cᵢ/(i+1) stay \
...
with multiplicity",
    doc := "the multiset of roots in the coefficient ring, with multiplicity — \
...
in an extension",
    ...
    -- own ring; the text lives here, at the declaration. Count-invariant
    -- grammar on purpose — "has 1 of its 5 roots" and "has 2 of its 5 roots"
    -- both read
    advisory := "{p} does not split over {ring}: it has {have} of its {deg} \
roots there (with multiplicity); `map {p} to ℂ[x]` reaches the rest" },
  ...
  { id := `ker, receiver := `MatrixElems,
    ...
    doc := "the kernel of the matrix read as a linear map, presented as a \
subspace by a reduced basis" },
...
is this one" },
  ...
  { id := `ker, receiver := `HomElems,
    ...
    doc := "the kernel of the hom — a subspace of its domain, presented by a \
reduced basis" },
  { id := `im, receiver := `HomElems,
    anchor := ``LinearMap.range,
    resultDoc := "the subspace φ(ℚⁿ) of the hom's codomain",
    doc := "the image of the hom — a subspace of its codomain, presented by a \
reduced basis" },
...
first-match trial in ℕ-order for a guarded subset",
    ...
    doc := "the element at an index of a stated enumeration — for ℤ and ℚ the anchor \
...
ceiling, not a general decision procedure)" },
  { id := `subset, receiver := `Sets, arity := 1,
    ...
    argDoc := "another set",
    resultDoc := "a boolean",
    doc := "inclusion — the same judgment `X ∈ 𝒫(A)` asks" },
...
is 2 and not 2i" },
  ...
  { id := `abs, receiver := `ComplexElems,
    ...
    doc := "the modulus |z|; on a real number, the absolute value" },
...
tolerance. `ℝ/O(ε)` is notation for that request, not a quotient — |a - b| < ε \
...
is the backend's own choice, named nowhere in the notation" },
...
Trusted: no finite exact computation on this side decides a limit, so the \
reply is checked for its kind only — an exact value, never a decimal. No \
ε-δ semantics is claimed; the limit is an operation whose answer is exact \
or a refusal" },
  { id := `definite_integral, receiver := `FunctionElems, arity := 2,
    anchor := ``intervalIntegral,
    argDoc := "the lower and upper bounds",
    resultDoc := "an exact value",
    doc := "the definite integral of the body between two bounds — trusted as \
the limit is, and refused rather than approximated when the answer is not \
among the exact values presented here" },
...
t — trusted as the limit and the definite integral are, with one more \
check: every coefficient must come back an exact rational, and one that \
does not is refused rather than turned into a decimal. The number of terms \
is a documented ceiling, not negotiated per call: a truncation past it is \
a loud refusal naming it" },
...
private def stdProfileRules : Array ProfileRule := #[
  ...
  -- the module fixture: ℤ/n is a cyclic ℤ-module, and enters at that — the
  -- most specific true membership. `Modules(ℤ)` follows by the inclusion
  -- edge, which is what carries `annihilator` down.
...
`annihilator` still reaches the fixture through `CyclicModules ≤ Modules`,
untransported, and the acceptance proofs below assert exactly that. -/
...
def acceptanceProofs (env : Environment) : CommandElabM Unit := do
  ...
  -- reaches the fixture through CyclicModules ≤ Modules — untransported,
  -- even though a functor out of Modules is registered: round one wins
  -- unconditionally, so round two can never take this method away
